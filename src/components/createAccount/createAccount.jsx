...
import {
  ERROR,
  CREATE_acent_ACCOUNT,
  acent_ACCOUNT_CREATED,
  DOWNLOAD_acent_KEYSTORE,
  acent_KEYSTORE_DOWNLOADED
} from '../../constants'
...
class Swap extends Component {
  ...
  componentWillMount() {
    ...
    emitter.on(acent_ACCOUNT_CREATED, this.acentAccountCreated)
    emitter.on(acent_KEYSTORE_DOWNLOADED, this.acentKeystoreDownloaded)
  };

  componentWillUnmount() {
    emitter.removeListener(ERROR, this.error);
    emitter.removeListener(acent_ACCOUNT_CREATED, this.acentAccountCreated)
    emitter.removeListener(acent_KEYSTORE_DOWNLOADED, this.acentKeystoreDownloaded)
  };
  ...
  acentAccountCreated = (account) => {
    this.setState({
      account,
      mnemonicWords: account.mnemonic.split(" ").sort(() => Math.random() - 0.5),
    })

    this.callDownloadacentKeystore(account.privateKey)
  };

  acentKeystoreDownloaded = () => {
    this.setState({
      page: 1,
      loading: false
    })
  ...
  onNext = (event) => {
    switch (this.state.page) {
      case 0:
        if(this.validateCreateAccountacent()) {
          this.callCreateAccountacent()
        }
        break;
      case 1:
  ...
  callCreateAccountacent = () => {
    dispatcher.dispatch({type: CREATE_acent_ACCOUNT, content: {} })
    this.setState({ loading: true })
  };

  callDownloadacentKeystore = (privateKey)  => {
    ...
    dispatcher.dispatch({type: DOWNLOAD_acent_KEYSTORE, content })
    this.setState({ loading: true })
  };
      ...
      <React.Fragment>
        ...
        <Grid item xs={ 12 }>
          <Checkbox
            id="accept"
            fullWidth={ true }
            label="I understand that acentridge cannot recover or reset my account details. I will make a backup of the account details and complete all wallet creation steps."
            value={ accept }
            error={ acceptError }
