...
class Store {
      ...
      function (payload) {
        switch (payload.type) {
          ...
          case GET_LIST_PROPOSAL:
            ...
            break;
          case GET_acent_BALANCES:
            this.getacentBalances(payload);
            break;
          case GET_ETH_BALANCES:
            this.getETHBalances(payload);
            break;
          case CREATE_acent_ACCOUNT:
            this.createAccountacent(payload);
            break;
          case DOWNLOAD_acent_KEYSTORE:
            this.downloadKeystoreacent(payload);
            break;
          case GET_ERC20_INFO:
  ...
  getacentBalances(payload) {
    const url = "/api/v1/getacentBalances"
    this.callApi(url, 'POST', payload.content, payload, (err, data) => {
      if(err) {
        console.log(err)
        emitter.emit(ERROR, err);
      ...
      if(data.success) {
        emitter.emit(acent_BALANCES_UPDATED, data.result);
      } else if (data.errorMsg) {
        emitter.emit(ERROR, data.errorMsg);
      } else {
        emitter.emit(ERROR, data.result);
  ...
  createAccountacent(payload) {
    const url = "/api/v1/createAccountacent"
    this.callApi(url, 'POST', payload.content, payload, (err, data) => {
      if(err) {
        console.log(err)
        emitter.emit(ERROR, err);
      ...
      emitter.emit(acent_ACCOUNT_CREATED, data.result);
    });
  };

  downloadKeystoreacent(payload) {
    const url = "/api/v1/downloadKeystoreacent"
    this.downloadFile(url, payload.content, (err, data) => {
      ...
      emitter.emit(acent_KEYSTORE_DOWNLOADED, data.result);
    });
  };
